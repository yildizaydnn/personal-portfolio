...
const RadialGradientBackground = ({ variant = 'hero', gradients = [] }) => {
    ...
    const generateGradient = (colors) => {
        ...
        return `radial-gradient(circle at center, transparent 0%, transparent 30%, ${colorStop}, transparent 70%, transparent 100%)`
    }
    ...
    return (
        <div className='absolute inset-0 overflow-hidden pointer-events-none'>
            {activeGradients.map((gradient, index) => (
                <div key={index} className={`absolute rounded-full pointer-events-none ${gradient.position} ${gradient.size}`}
                    style={{
